...
Dans le chapitre précédent, nous avons présenté différents tracés permettant d'évaluer comment l'architecture de cartes extrait une représentation interne du modèle d'entrée lors de l'apprentissage sur des cartes et données en une dimension.
Cet apprentissage du modèle se caractérise notamment par le fait que chaque carte dissocie les BMUs en fonction du modèle d'entrées global et non seulement de son entrée externe. 
...
Nous proposerons dans ce chapitre deux indicateurs permettant d'évaluer l'apprentissage du modèle par l'architecture de cartes. 
...
L'hypothèse que l'apprentissage du modèle se traduit dans chaque carte par le fait que $U$ est une fonction du BMU dans chaque carte reste à vérifier, en particulier en dimension supérieure à 1. Le calcul de l'information mutuelle entre les éléments de réponse d'une carte et ses entrées est pertinent à explorer pour compléter la compréhension du modèle. L'information mutuelle est par ailleurs largement utilisée comme métrique, indicateur ou fonction de coût dans de nombreux domaines, donc un grand nombre de méthodes d'estimation et d'implémentation existent pour son calcul sur des données statistiques.
...
Nous présenterons d'abord un indicateur s'appuyant sur le ratio de corrélation, qui répond spécifiquement à nos besoins lorsque les cartes sont en une dimension. 
Nous explorons ensuite un indicateur s'appuyant sur l'information mutuelle, que nous avons publié en \cite{Gonnier2020ConsensusDS}.  
Cet indicateur permet de mesurer une dépendance fonctionnelle entre les deux variables mais dépend beaucoup de la méthode d'estimation utilisée et est limité si $U$ est de grande dimension. L'estimation de l'in
...
Remarquons que l'entropie de Shannon concerne des variables discrètes uniquement.
Shannon a cependant proposé une autre version de l'entropie est définie pour des variables continues, l'entropie différentielle~:
\begin{equation}
    H(X) = - \int_{x \in E_X}{p_X(x)\textrm{log}(p_X(x))dx}
\end{equation}
...
L'entropie de Shannon se mesure en $bit/symbole$ (pour un logarithme en base 2)
...
Outre les entropies jointes et conditionnelles, l'existence d'une relation statistique entre deux variables aléatoires $X,Y$ à valeurs dans $E_X,E_Y$ se mesure par \emph{l'information mutuelle}.
...
\begin{equation}
    I(X,Y) = \int_{x \in E_X}\in _ {y \in E_Y }{p_{XY}(x,y)\textrm{log}(\frac{p_{XY}(x,y)}{p_X(x)p_Y(y)})dx \: dy}
\end{equation}

Contrairement à l'entropie, la valeur de l'information mutuelle pour des variables continues correspond bien à une limite des valeurs de l'information mutuelle discrète, pour un nombre de catégories tendant vers l'infini.
...
\item Elle s'exprime à partir de l'entropie de Shannon : $I(X,Y) = H(X) + H(Y) - H(X,Y) = H(X) - H(X|Y) = H(Y) - H(Y|X)$
...
Ces propriétés restent vraies pour des variables continues, en prenant $H$ l'entropie différentielle.
...
Nous cherchons à définir un indicateur permettant d'évaluer l'apprentissage du modèle par l'architecture de cartes. D'après les observations réalisées sur des cartes en une dimension, l'existence d'un apprentissage du modèle complet par une carte se traduit par le fait que $U$ est une fonction de $\bmu$ \emph{dans chacune des cartes}.
Le ratio de corrélation nous permet de mesurer cette dépendance fonctionnelle. Toutefois, son estimation ne prend pas en compte la continuité des valeurs de $p$.

\cite{ShwartzZiv2017OpeningTB} : emploi de IM pour évaluer des autoencoders.
...
L'\emph{information mutuelle} $I(\bmu, U)$ entre $U$ et le BMU d'une carte est une mesure plus générale permettant d'évaluer l'information qu'une position du BMU d'une carte porte en moyenne sur la valeur de $U$, et $U$ sur le BMU. Nous avons donc exploré son utilisation en terme d'évaluation du modèle.
...
Nous considérons ici des représentations paramétriques d'un modèle d'entrée étant en bijection avec le modèle d'entrée. Si un seul des paramètres du modèle nous intéresse, les représentations et indicateurs proposées dans ce chapitre restent valables comme indicateur de l'apprentissage d'une relation entre entrée des cartes.
