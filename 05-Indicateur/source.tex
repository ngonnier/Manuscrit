...
% \begin{table}
%     \begin{tabular}
%         Distribution & Entrées & Cartes indépendantes & CxSOM \\
%         Cercle 2D & & & \\
%         Lissajous & & & \\
%         Plan & & & 
%         Anneau & & & \\
...
Calcul possible en 2D : mettre la formule.
...
L'estimateur de Kraskov est plus précis que l'estimateur par histogrammes et est moins sensible aux paramètres choisis pour son estimation, en l'occurence le nombre de voisins considérés.
Il est notamment plus performant que l'approche par histogrammes en grande dimension.
...
Nous décrivons ici une méthode que nous avons proposée avant de s'intéresser au ratio de corrélation. Ce dernier sera finalement à privilégier par rapport à cette méthode pour montrer la relation fonctionnelle entre $U$ et $\bmu$. 
...
\subsubsection{Choix des paramètres d'estimation}
...
L'estimation de l'information mutuelle continue par la méthode de Kraskov donne quant à elle une valeur précise de la quantité statistique qu'est le coefficient d'incertitude entre $U$ et $\bmu$.
Nous pourrons considérer cette quantité comme un deuxième indicateur différent de $I_x$.
...
\section{Comment utiliser l'information mutuelle continue comme indicateur d'un apprentissage au sein de cartes connectées ?}

\subsection{\'Evolution de l'information mutuelle entre $U$ et $\bmu$ au cours d'un apprentissage}

L'indicateur proposé en section précédente, comme le ratio de corrélation, représente le fait que $U$ est une fonction du BMU dans chaque carte.
Plus généralement, il sera intéressant d'étudier l'information mutuelle entre $U$ et les BMUs $\bmu\m{1}, \bmu\m{2}$.
En figure~\ref{fig:MI_evol_total}, nous traçons l'évolution de l'information mutuelle dans les deux cartes, estimée par la méthode des KNN. Nous observons que l'information mutuelle entre $U$ et $\bmu$ converge vers une valeur plus élevée dans une carte isolée que dans une architecture CxSOM.
...
Des méthodes permettant de séparer l'information entre variables existent dans la littérature~; nous en discuterons en section suivante. Elles nous permettraient ainsi de mesurer le gain d'information sur $U$ dans une ou plusieurs cartes sans s'intéresser à l'information apprise sur l'entrée externe $\inpx$.
...
La perte d'information globale sur le modèle d'entrée reste à généraliser pour des distributions d'entrée différentes.
...
La mesure de l'information mutuelle est cependant bien plus large que le seul calcul de $I(U,\bmu)$~; de nombreux aspects nous semblent intéressant à explorer pour une compréhension de l'apprentissage du modèle dans des architectures comportant plus de cartes. Par ailleurs, la méthode par KNN présentée dans ce chapitre est une méthode classique d'estimation de l'information, mais il existe de nombreuses autres méthodes d'estimation \cite{Doquire2012ACO}. Des méthodes ont également été développées pour la mesure de l'information mutuelle entre variables continues et discrètes \cite{ross_mutual_2014, Gao2017EstimatingMI}. Enfin, l'information mutuelle a été utilisée pour analyser l'apprentissage dans des réseaux de neurones profonds en \cite{ShwartzZiv2017OpeningTB} ou encore directement comme métrique d'apprentissage en \cite{Hjelm2018LearningDR}.
Cette grandeur est donc bien documentée.

Tout d'abord, il est possible de s'intéresser à la notion d'information mutuelle multivariée~: étant donné une variable cible $S$ et deux variables $R_1$ et $R_2$, $I(S;R_1,R_2)$ est l'information mutuelle multivariée entre $S$ et la variable jointe $(R_1,R_2)$ Nous pourrons ainsi mesurer, dans une architecture de cartes, $I(U; \bmu\m{1}, \bmu\m{2})$ par exemple.
Il est également possible de décomposer cette information entre les entrées $U$, $\inpx\m{1}$ et $\inpx\m{2}$
\'A partir de cette information multivariée \cite{williams_nonnegative_2010} définit, en plus de l'information mutuelle, la notion de redondance et de synergie entre variables, illustrée en figure \ref{fig:redondance}.
La redondance est l'information sur $S$ portée à la fois par $R_1$ et par $R_2$, et la synergie l'information portée seulement par la jointure des variables $R_1$ et $R_2$
...
Des travaux comme \cite{lizier_detecting_2007,ceguerra_information_2011} s'intéressent quant à eux à la notion de transfert d'information au sein de systèmes dynamiques complexes. Le calcul d'information entre les éléments des cartes pourrait ainsi également s'appliquer à la quantification de la dynamique d'apprentissage d'une architecture de cartes.

La représentation des éléments d'une carte et des entrées d'un point de vue statistique que nous avons proposé dans cette thèse est une méthode pertinente pour la compréhension des comportements d'apprentissage du modèle CxSOM et leur analyse statistique reste à explorer dans des travaux futurs.
