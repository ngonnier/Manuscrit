...
Cependant, peu de travaux ont exploré l'aspect topologique et la simplicité des règles de calcul d'une carte pour les assembler en architectures modulaires comportant des rétroactions, formant un système dynamique et conférant à une architecture de cartes un comportement de prise de décision.
...
Après quelques millisecondes, une activité émerge dans le cortex auditif au niveau des neurones sensibles à l'emplacement spatial de la source du stimulus visuel, témoignant d'une interaction entre les cortex visuels et auditifs~\cite{lefort_apprentissage_2012}.
...
% \begin{figure}
%     \centering
%     \includegraphics[width=\textwidth]{Cappe_bio.pdf}
%     \caption{A. Schéma de connexions entre aires sensorielles existant dans le cortex du ouistiti et du macaque, faisant apparaître des connexions rétroactives entre aires. B. Schéma conceptuel décrivant le prinipe de traitement hiérarchique de l'information multisensorielle dans le cortex, tout en incluant des connexions bas-niveau. Modifié à partir de \cite{Cappe2009MultisensoryAP}\label{fig:cappe}}
% \end{figure}
...
% \section{Les Champs Neuronaux Dynamiques, un modèle d'activité bio-inspiré}

% \subsection{Théorie des champs dynamiques (DFT)}

% \subsection{Architectures de DNF~: un modèle dynamique exhibant un comportement émergent}
...
Un module se définit comme un élément possédant des règles d'évolution à partir d'entrées, et une sortie. L'assemblage de ces modules par une interface définie constitue une architecture modulaire. D'après la définition de la modularité en informatique, des modules doivent pouvoir être ajoutés ou retirés à une architecture modulaire sans modifier la structure interne des autres modules de l'architecture.
...
Nous appelons une architecture par sélection un ensemble de cartes organisées de façon hiérarchique et dont la sortie d'une carte permet de diviser l'espace d'entrée en sous-espaces qui seront utilisés par le niveau de cartes suivant.
Détaillons par exemple l'architecture développée en \cite{barbalho_hierarchical_2001}, représentée en figure \ref{fig:hsom_selective}. Ce modèle est également décliné en une version dynamique dans laquelle les paramètres des cartes dépendent des données, en \cite{Costa2016ANS}.
...
Lors la deuxième phase de l'apprentissage, les données d'entrées sont réparties en plusieurs sous-ensemble, tel que chaque sous-ensemble $\mathcal{D}_i$ est l'ensemble des entrées $X_t$ ayant $i$ pour position du BMU associé à l'entrée.
...
Seul ce processus traite l'information de chacune des cartes et apporte une connexion entre cartes. Dans ce sens, on ne peut pas parler d'architecture modulaire de par l'aspect global de l'algorithme. 
...
Ces modèles apparaissent comme des architectures modulaires idéales, car aucun a priori n'est associé aux modules, même une fois connectés en architecture. Leur spécialisation intervient uniquement grâce à leurs règles d'évolution internes.
...
L'interface entre cartes utilisée en A-SOM est donc le champ d'activation des neurones. Cette interface par transmission d'activation comme entrée d'une carte est équivalente à des connexions pondérées par des poids $\w_{cij}$ reliant le neurone $i$ d'une carte au neurone $j$ de l'autre. On a alors $w_c(i) = [\w_{ci1}, \cdots, \w_{ciN}]$.
...
La notion de BMU se définit alors par le neurone s'activant le plus fortement à la présentation d'une entrée, inhibant alors les autres neurones de la carte grâce à des calculs de champs neuronaux dynamiques.
Nous pouvons souligner le parallèle existant entre la version cellulaire et la version centralisée d'une carte de Kohonen. Les champs neuronaux dynamiques implémentent en effet un mécanisme de \emph{Winner Take All} similaire au choix du BMU par argmax dans une carte classique. Dans nos travaux, nous étudions les cartes de Kohonen classiques, mais les mécanismes occurrant à l'échelle d'une carte dans une carte de Kohonen cellulaire ressemblent à ce qu'on observe dans une carte classique.
...
L'architecture proposée en \cite{baheux_towards_2014} cherche a transformer le modèle cellulaire de Bijama en s'appuyant sur des cartes auto-organisatrices classiques et l'applique au traitement de séquence. Cette architecture, décrite en figure~\ref{fig:baheux}, est composée de deux cartes. 
...
\begin{table}[t!]
    ...
    \begin{tabular}{p{2cm}p{6cm}p{2cm}p{5cm}}
        ...
        SOMSD       \tnote{13}& Récurrente                       &               & Position du BMU \\
        \hline
