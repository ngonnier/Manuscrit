...
Dans cette section, nous cherchons à comparer des modèles existants d'architectures de cartes d'un point de vue structurel, afin d'en tirer une taxonomie générale. Nous présenterons les différents modèles sous la notation unifiée, introduite en~\ref{sec:modele_som}, afin d'offrir une vue synthétique du domaine.
L'analyse de ces modèles nous permettra de situer dans la littérature l'architecture que nous développons, et d'émettre des hypothèses concernant les comportements qu'on peut en attendre.
...
L'étude des architectures développées dans les travaux précédents nous amène à différencier les structures selon plusieurs aspects, résumés en figure~\ref{fig:taxo}. 
...
Enfin, la temporalité de l'algorithme de mise à jour de l'architecture peut se présenter de différentes façons. Nous appelons séquentielle un algorithme dans lequel l'apprentissage est complètement effectué sur un module, avant de mettre à jour les suivants. La mise à jour est synchrone lorsqu'on peut définir des itérations communes à l'architecture, pendant lesquelles tous les modules seront mis à jour. Enfin, la mise à jour est asychrone lorsqu'un module n'est mis à jour que lorsqu'un signal déclencheur lui parvient.
...
Nous appelons architecture par sélection un ensemble de cartes organisées de façon hiérarchique et dont la sortie d'une carte permet de diviser l'espace d'entrée en sous-espaces qui seront utilisés par le niveau de cartes supérieur.
