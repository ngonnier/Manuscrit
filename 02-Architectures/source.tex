...
L'aspect bio-inspiré se retrouve également dans les motivations des modèles robotiques. Ces modèles se placent en effet dans le paradigme d'embodiment (\emph{Embodied Cognition}), dont parmi les principes de base figurent la fusion de données multimodales, le traitement de séquences et l'apprentissage incrémental \cite{Smith2005TheDO}, inspiré de la biologie. 
...
La motivation biologique de telles architectures s'appuie sur le principe de réentrée, qui postule que les neurones d'aires cérébrales sensorielles ou motrices sont directement connectés à des neurones d'autres aires. Cette vision de la connectivité du cerveau est appuyée par des travaux récents de neurosciences, passés en revue en ???
Un modèle d'architecture décentralisé est intéressant pour la recherche de nouveaux comportements dans la mesure où il n'impose pas de structure spécifique pour l'architecture.
...
Ce modèle de connexions hebbienne entre deux cartes est également proposé en \cite{jayaratne_bio-inspired_2018}.
...
%TABLEAU : SOM hiérarchiques par transmission + SOM non hiérarchiques + SOM récurrentes
% onglets : H/NH, maj seq/synchrone, elements transmis

\begin{table}
    \small
    \begin{tabular}{p{2cm}p{4cm}p{2cm}p{4cm}}
        Modèle                                  & Type                                  & Mise à jour      & Mode de transmission    \\
        HSOM\footnote{\cite{lampinen_clustering_1992,hagenauer_hierarchical_2013,Paplinski2005MultimodalFS}}   & Hiérarchique                          & Séquentielle     & Positions BMU           \\
        Deep SOM\footnote{\cite{Liu2015DeepSM},\cite{wickramasinghe_deep_2019}} & Hiérarchique       & Séquentielle    & Positions BMU   \\
        \hline
        MMCM \footnote{\cite{dominey13}}                  & Non-hiérarchique, centralisée           & Séquentielle    & Positions BMU    \\                  
        SOIMA\footnote{\cite{escobar-juarez_self-organized_2016}} & Non-hiérarchique, centralisée    & Séquentielle    & Champ d'activité \\
        Bijama\footnote{\cite{menard05}}                    & Non-Hiérarchique, centralisée         &                 & Champ d'activité partiel \\ 
        A-SOM  \footnote{ \cite{johnsson_associating_2008}}  & Non-Hiérarchique, décentralisée       & Synchrone       & Champ d'activité \\
        SOMMA  \footnote{ \cite{lefort_active_2015}}          & Non-Hiérarchique, décentralisée       & Synchrone       & Champ d'activité partiel \\
        ...
        \hline
        RSOM  \footnote{\cite{varsta_temporal_2001}}       & Récurrente                            & x               & champ d'activité \\
        MSOM   \footnote{\cite{Strickert2005MergeSF}}& Récurrente                                   & x               & poids du BMU\\
        A-SOM   \footnote{\cite{Buonamente2013SimulatingAW}}& Récurrente                            & x               & Champ d'activité\\
        Recursive SOM \footnote{\cite{Voegtlin2002RecursiveSM}}& Récurrente                            & x               & Champ d'activité\\
        SOMSD       \footnote{ \cite{hammer_self-organizing_2005}}& Récurrente                       & x               & Position du BMU \\

    \end{tabular}
\end{table}

Nous avons détaillé la littérature existante en terme d'architecture de SOMs. Nous avons divisé ces architectures en deux grandes catégories~: un format hiérarchique et un format non-hiérarchique incluant des rétroactions.
Le format hiérarchique implique un apprentissage couche par couche. Ce format est très appliqué et permet d'améliorer les capacités de clustering d'une SOM classique, principalement dans le cas ou les données traitées présentent elles-mêmes une structure hiérarchique, telles que des images ou des phrases.
...
Nous avons remarqué que seulement peu de travaux ont exploré l'idée d'associer des SOMs en architectures comportant des rétroactions. Parmi ces quelques travaux, les interfaces entre cartes considérées par leurs auteurs s'appuient plutot sur la transmission de champ d'activation, et la gestion des rétroactions est effectuée de façon séquentielle, sauf pour le modèle A-SOM.
Pourtant, la transmission de la position du BMU comme information entre carte apparait comme un paradigme souvent utilisé dans le domaine des cartes hiérarchiques, qui exploite pleinement l'aspect topologique de la carte de Kohonen, est indépendante du type d'entrée fourni à une carte et est une valeur de faible dimension, donc intéressante pour le calcul. Ce paradigme permet aux architectures s'y appuyant de réaliser de bonnes performances en terme d'apprentissage automatique.
Seuls les travaux de \cite{dominey13} s'appuient sur la position du BMU dans le cadre des SOM non-hiérarchiques, et nous n'avons relevé aucun modèle cherchant à implémenter une architecture décentralisée s'appuyant sur la position du BMU comme information transmise entre cartes.

D'autre part, la plupart des architectures relevées dans la littérature s'appuient sur des mises à jour séquentielles. Dans le cadre futur d'une application en 
Dans cette thèse, nous cherchons ainsi à nous appuyer sur la transmission de la position du BMU pour construire des architectures non-hiérarchiques décentralisées.
