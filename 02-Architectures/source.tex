...
\item Le n\oe{}ud ayant le prototype le plus proche de $\inpx$ selon une distance $d(\w, \inpx)$ est choisi comme \emph{Best Matching Unit} (BMU) de la carte. Son indice est noté $\bmu$. \begin{equation}
    \bmu = \argmin d(\w,\inpx)
\end{equation}
Ce calcul de distances, utilisé dans la version originale de SOM, est remplacé dans d'autres modèles de cartes par le calcul d'une \emph{activation} $a(\inpx, \w(p))$ liant les poids des n\oe{}uds et les entrées. 
...
L'algorithme de Kohonen repose donc à la fois sur un mécanisme de compétition par la sélection de la BMU de la carte et un processus de coopération avec le déplacement des unités voisines de la BMU, aussi nommé \og Winner Take Most \fg{}.

Le processus de mise à jour des poids d'une carte de Kohonen se traduit par un dépliement de la carte dans l'espace d'entrée. On parlera donc aussi de \emph{dépliement} d'une carte lorsque qu'on parle d'apprentissage. Ce dépliement est représenté en figures \ref{fig:som2d} et \ref{fig:som1d} pour des exemples de cartes en une et deux dimensions, se dépliant sur des données en deux dimensions.
...
Par exemple, la figure \ref{fig:digits} représente le dépliement d'une carte sur des imagettes MNIST.
...
Des exemples de topologies de cartes en ligne et grilles sont présentées en figure~\ref{fig:topo}.
Par sa topologie, une carte de Kohonen permet d'extraire une \emph{représentation} de l'espace d'entrée en faible dimension, dans la répartition de ses poids.
Chaque entrée peut alors être représentée par un vecteur 1D ou 2D, qui est la position du Best Matching Unit correspondant dans la carte.
...
La construction d'une architecture de carte auto-organisatrices a été développée dès la création du modèle. Par contre, la structure des architectures est variées.
Dans cette section, nous cherchons à comparer des modèles existants d'architectures de cartes d'un point de vue structurel, afin d'en tirer une taxonomie générale.
...
Nous nous intéressons à des modèles d'architectures modulaires.
Par module, nous entendons comme un élément possédant des règles d'évolution temporelles à partir d'entrées et une sortie. Nous définissons comme architecture modulaire l'assemblage de modules de même type par une interface définie. Des modules doivent pouvoir être ajoutés ou retirés à une architecture modulaire sans modifier la structure interne des autres modules de l'architecture.
Les architectures auxquelles nous nous intéressons sont celles dont les modules sont des cartes auto-organisatrices, dont le modèle a été adapté pour permettre la construction d'architectures. 
...
Les modules d'une architecture n'ont pas de rôle défini a priori avant d'être insérés dans l'architecture, mais seulement des règles d'activation et de mise à jour.
Les variables du module qui sont accessibles à d'autres modules de l'architecture sont appelées les sorties du module. Dans le cas des SOMs, il peut s'agir de la position du BMU, des poids ou des valeurs d'activités.
Ces variables constituent l'interface entre cartes.


\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{module.pdf}
    \caption{Description d'une carte de Kohonen en tant que module d'une architecture modulaire. Un module prend des entrées et possède des variables d'état, ici les poids $\w$, dont l'évolution dans le temps est régie par des règles de mise à jour ou équations d'évolution. La sortie d'un module est un ensemble de variable d'états, accessible par d'autres modules. Les variables d'état d'une carte sont, entre autres, le BMU ou l'activation. \label{fig:module}}
\end{figure}
