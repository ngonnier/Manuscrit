...
L'aspect global du processus de connexion des cartes dans ce type d'architecture nous intéresse moins d'un point de vue modulaire. Ces architecturessont plutot des agrégations de cartes de Kohonen dont on étudie les
...
Ces travaux implémentent des structures qui rejoignent celles présentées précédemment, mais puisent leur inspiration des réseaux de neurones profonds (\emph{Deep Learning}), ayant notamment connu leur essor avec les réseaux convolutionnels permettant l'apprentissage supervisé d'images \cite{lecun_deep_2015}.
...
La façon d'associer ces cartes en architecture rejoint cependant les SOMs hiérarchiques par transmission de représentation par leur mécanismes de transmission d'information.
...
D'autres travaux implémentent les interfaces par transmission du poids du BMU, comme \cite{dozono_convolutional_2016}. Les auteurs de ces travaux, comme D-SOM, décomposent l'image d'entrée en imagettes qui sont utilisées en tant qu'entrées d'une couche de cartes. Après apprentissage de cette couche, l'image est reconstruite grâce aux poids des BMU, puis décomposée en imagettes de tailles différentes pour être soumise à la deuxième couche de carte.

et \cite{mici_self-organizing_2018} s'appuient sur des cartes hiérarchiques pour effectuer de la fusion de données spatio-temporelles.
...
Le champ d'application des architectures ascendantes est le même qu'une SOMs classique~: quantification vectorielle et classification. 
Notons par contre que le type d'architecture définit le champ d'application~: 
