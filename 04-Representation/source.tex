...
L'algorithme CxSOM que nous avons proposé définit des règles d'apprentissage pour construire des architectures non-hiérarchiques de cartes auto-organisatrices.
Dans cette thèse, nous voulons étudier les propriétés d'organisation et d'apprentissage de l'architecture CxSOM. Nous devons donc définir un cadre d'étude, et avons choisi de nous placer dans un cadre de mémoire associative.
L'objectif de la mémoire associative pour une architecture de cartes est d'apprendre une représentation des relations existant entre des entrées de différentes modalités, tout en apprenant une représentation de chaque espace d'entrée au sein des cartes.

L'objectif de cette thèse est ensuite d'observer comment l'organisation qui émerge au sein des cartes de la structure traduit cet apprentissage associatif.
Pour analyser l'organisation de ces cartes, nous avons besoin de d'introduire de nouvelles représentations par rapport à celles utilisées dans les SOM classiques. 
Ce chapitre présente ces représentations, que nous utiliserons dans toutes les expériences présentées par la suite dans ce manuscrit.
Elles ont pour but de qualifier la qualité de l'apprentissage et de mettre en évidence l'apprentissage des entrées et des relations entre entrées au sein de l'architecture.
...
% Un échantillon test donne un ensemble de valeurs de variable jointe $(\inpx\m{1}, \cdots, \inpx\m{D}, \bmu\m{1},\cdots, \bmu\m{n})$.
