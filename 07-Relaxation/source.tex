...
Dans ce chapitre, nous chercherons à observer le comportement de la relaxation et notamment sa convergence.
Avant cela, nous nous intéressons à son inspiration biologique et pourquoi il reste proche des architectures de cartes cellulaires développées précédemment dans l'équipe \cite{khouzam_neural_2014,menard05}, qui ont inspiré le modèle CxSOM.

\subsection{Du calcul cellulaire au calcul à l'échelle d'une carte}

Dans les modèles d'architectures développés chez \cite{khouzam_neural_2014,menard05} le processus dynamique d'apprentissage s'appuyait sur des champs neuronaux dynamiques couplés (DNF), qui remplacaient la recherche de BMU.
...
\`A partir de l'équation~\ref{eq:opti}, interrogeons-nous sur les conditions à remplir pour le processus de relaxation et sa valeur finale pour qu'il permettent de trouver une \og Best Matching Unit \fg{} qui ait un sens pour l'apprentissage.
Tout d'abord, on attend évidemment de la relaxation qu'elle converge pour pouvoir parler de recherche de BMU. Bien que nous ayons fixé une limite d'itérations $\tau_{max}$, une valeur trouvée à l'issue des $\tau_{max}$ n'a pas de sens pour l'apprentissage.
Nous voudrons donc d'abord vérifier que la recherche du BMU converge.
...
Ce chapitre présente des éléments de mesure expérimentale de ce processus de relaxation, réalisée sur des cartes 1D et 2D prenant en entrée un jeu de données 1D.
L'évaluation de la convergence sera réalisée sur des exemples d'apprentissage d'une architecture de deux et trois cartes en section~\ref{sec:relax_conv}.
...
On attend d'un algorithme de recherche du BMU que la valeur trouvée en sortie soit uniquement relative aux poids de la carte et aux entrées~: elle ne doit pas dépendre de l'initialisation du processus de relaxation.
Dans ce sens, nous voulons observer si la valeur trouvée à l'issue de la relaxation, à savoir le point de convergence s'il existe, ne dépend pas des conditions initiales de la relaxation.
...
En section~\ref{sec:relax_expe} nous étudierons empiriquement si la fonction $\mathbf{f}$ générant la suite $\mathbf{\bmu}_\tau$ admet un point fixe, et si la suite converge vers une unique solution indépendamment des valeurs d'initialisation.
...
% Nous observerons qu'il n'existe pas toujours un unique point fixe, déjà pour des architectures de deux cartes, en particulier lorsque les poids sont répartis aléatoirement au début de l'apprentissage. La relaxation n'apparaît alors pas converger.
% Cependant, il semble que ces cas de non-convergence n'influencent pas le dépliement ultérieur des cartes.
% Nous observerons qu'à la fin de l'apprentissage, la disposition des poids obtenue dans chaque carte semble permettre l'existence d'un unique point fixe et que la relaxation converge vers ce point, indépendamment de l'initialisation des valeurs de $\bmu\m {i}_{\tau}$. 
% Cet unique point fixe est alors la solution du problème~\ref{eq:opti}.
...
\`A partir de ces valeurs, nous traçons l'évolution du nombre de pas moyen nécessaire à la convergence au cours de l'apprentissage, en prenant en compte les cas dans lesquels la relaxation ne converge pas, sur chaque phase de test $t$.
...
Nous avons essayé d'éviter le premier cas en fixant la limite $\tau_{max}$ assez grande par rapport à la taille de la carte~: les cartes sont de taille 500 et le pas d'évolution de la relaxation d'une dizaine d'unités. 
...
Toutefois, le fait que la relaxation ne converge pas en début d'apprentissage ne perturbe pas l'organisation des poids, car nous avons pu observer dans la suite de nos expériences que les poids des cartes évoluent vers des états organisés et stables.
...
Cette observation renforce l'intérêt d'initialiser la relaxation au maximum de l'activité externe et non aléatoirement dans l'espace des positions, afin de contraindre un peu plus l'évolution de la relaxation.
