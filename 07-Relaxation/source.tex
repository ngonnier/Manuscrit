...
\begin{document}
\chapter{Analyse du mécanisme de relaxation}
...
L'algorithme de relaxation proposé dans le modèle CxSOM prend comme inspiration sur le modèle de relaxation développé dans l'équipe pour créer des architectures de cartes auto-organisatrices cellulaires \cite{khouzam_neural_2014,menard05}.
...
Les DNF, introduits en \cite{Amari1977DynamicsOP} permettent de modéliser à gros grains l'activité spatiotemporelle d'une population de neurones impulsionnels. Au lieu de s'intéresser à l'activité d'un seul neurone, tel que son taux de décharge, le modèle des DNF calcule un potentiel $u(x,t)$ en un ensemble de positions continues $x$ de la population de neurones, à un temps $t$, réagissant à un stimulus $s(x)$. Ce calcul d'activité est une approximation du taux de décharge moyen du champ de neurones.
$u(x,t)$ s'exprime à partir d'une équation différentielle~; son évolution prend en compte le potientiel $u(y,t)$ aux positions voisines de $x$, pondérées par un terme d'interaction latérale. Ce terme est en général une différence de gaussiennes, rendant le voisinage proche de $x$ \emph{inhibiteur}~: son potentiel influe négativement sur le potentiel $u(x,t)$, tandis que le voisinage à plus longue distance est excitateur.
Le calcul du potentiel des DNF sur un pattern d'entrée résulte en la formation d'une bulle d'activité réagissant au signal d'entrée $x$. Les DNF ont plusieurs applications possibles comme le filtrage de bruit ou le suivi d'une cible.
Pour une entrée constante, la dynamique du DNF mène à la formation d'une bulle se stabilisant à une position représentant le mieux le signal d'entrée.
