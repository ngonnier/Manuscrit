...
\begin{document}
\chapter{Exploration et représentation du mécanisme de relaxation}\label{chap:relaxation}
...
Afin de valider son utilisation en tant que méthode de recherche de BMU et d'interface entre carte, nous cherchons dans ce chapitre à décrire son comportement.

\subsection{Origine du mécanisme de relaxation~: passage du calcul cellulaire au calcul à l'échelle d'une carte}

Le mécanisme de relaxation que nous proposons, qui s'appuie sur le calcul d'activation et le déplacement de BMUs, hérite des architectures de cartes cellulaires développées précédemment dans l'équipe \cite{khouzam_neural_2014,menard05}, qui ont inspiré le modèle CxSOM.
...
Les champs neuronaux dynamiques, introduits en \cite{Amari1977DynamicsOP} sont des systèmes dynamiques temporelles modélisant à gros grains l'activité spatiotemporelle d'une population de neurones impulsionnels. 
...
\begin{figure}
	...
	Cet exemple de DNF implémente ainsi un mécanisme de Winner Take All. En fonction des paramètres, les DNF peuvent présenter d'autres réponses. \label{fig:DNF}
	}
\end{figure}
...
Les calculs permettant le mécanisme Winner Take All implémenté par le DNF se rapprochent de calculs cellulaires~: ils ne dépendent que d'un voisinage de chaque position.
...
Grâce à cet aspect dynamique et cellulaire, la construction d'architectures de cartes auto-organisatrices couplées en~\cite{khouzam_neural_2014,menard05} utilise des DNF couplés associés aux différentes cartes. Leur évolution couplée, appelée relaxation dans ces travaux, amènent les DNF à se stabiliser sur des bulles d'activité, définissant dans chaque carte les unités qui seront mise à jour.
...
Cependant, l'optimisation des paramètres du DNF est lourde à réaliser \cite{fix:hal-00869726}, et le calcul du potentiels est extrêmement gourmands en calculs. 
...
Dans le même esprit que l'utilisation d'une activation chez Kohonen pour trouver le BMU au lieu d'un mécanisme cellulaire, l'algorithme de relaxation que nous avons proposé pour CxSOM cherche à conserver un processus dynamique de recherche de BMU mais en plaçant les calculs à l'échelle d'une carte. 
...
Dans notre modèle, le mécanisme de relaxation permet à la fois à trouver un BMU dans chaque carte, c'est-à-dire une position à laquelle une activité est maximale servant à la mise à jour, et joue le rôle d'interface entre cartes.
...
Nous proposons d'abord une formalisation plus détaillée qu'au chapitre \ref{chap:modele} de l'algorithme de relaxation, qui permettra de mettre en valeur les éléments intervenant dans la dynamique du système.
Nous définirons notamment l'équivalent pour l'architecture CxSOM de la recherche d'argmax d'une carte simple.
La formalisation de l'algorithme nous permettra finalement de proposer les conditions de convergence que la relaxation doit présenter afin de pouvoir être considérée comme une recherche de BMU.

Nous verrons que comportement de la relaxation dépend complètement des configurations des poids externes et contextuels des cartes de l'architecture, qui résultent d'un processus de mise à jour et n'ont pas de formulation analytique. 
Afin d'explorer les conditions de convergences formalisées, nous chercherons à visualiser et caractériser des trajectoires de relaxation sur des exemples de configuration de poids, à différents instants de l'apprentissage.
Sans chercher à apporter une preuve de convergence, nous proposerons des hypothèses et intuitions à partir des exemples tracés dans ce chapitre.
