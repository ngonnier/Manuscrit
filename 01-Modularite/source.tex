...
The notion of emergent computation has been introduced in the 90s to describe a system composed of several agents, following rules and interacting one to another \cite{Forrest1990EmergentCS}. As the interactions are non-linear, the global system has complex behavior emerging from the interaction of the components. It is a complex system.
This notion has been applied to a wide range of computing concepts such programming languages, database search or parallel computing. 
In the field of machine learning algorithms, emergent computation sounds in the field of unconventional computing. Swarm intelligence, \cite{Bonabeau1999SwarmI} for example, applies emergent processes to machine learning. Simple agents following learning rules could, by collaboration, globally present complex learning abilities. Let us cite as example biological and artificial neurons. 
Complex systems are described by their capacity of self-organization, which is the complex behaviour and patterns emerging from the entire system. In seems then natural to explore how to assemble learning structures already based on self-organisation rules or local computation. The main self-organization algorithms are, on one hand, neuron populations reacting to local firing rules, like STDP. An other category is self-organizing maps \cite{Kohonen1982} and its many variants ( growing maps [ref], tree-structured SOM [ref] ). These algorithms are directly inspired by the functioning of the human brain. Indeed, many areas of the brain, sensory and associative, are modeled by topographic maps [ref]. Associating self-organizing algorithm has then a  direct biological inspiration.
Then, a great example of how the association of several models has lead to more complex system is deep learning : altough perceptrons were in theory able to represent most systems, their combination in multi layer neural networks has opened a lot of new possibilities for machine learning. 

Finally, Emergent computation is then still a field to explore with the development of neuromorphic substrates.

In this work, we explore several architectures based on self-organizing neural networks. We investigate in particular

\section{Définir la modularité}


\section{Avantages des systèmes modulaires}
...
\subsection{Architectures de cartes auto-organisatrices}
...
Certains travaux développent des cartes modulaires où les connexions entre cartes sont créées neuronalement. C'est le cas de l'algorithme SOIMA \cite{SOIMA}, où les connexions entre neurones sont apprises avec une règle hébraïque alors que l'apprentissage se fait dans chaque carte : les neurones qui s'activent ensemble voient leur poids de connexion renforcé.

Traduit avec www.DeepL.com/Translator (version gratuite)

\subsection{Discussion}
There are several way to connect maps together. 
One is by combining activity calculation, such as RSOM, A-SOM.

The last one is by transmitting a element from a map as an input to another one; it can be whole set of activities, such as voeglin, weights (MSOM), or the index of the best matching unit (dominey, SOMSD, CxSOM). The transmission of only the best matching unit index is particularly interesting, as it reduces the state of a map to a small-dimension vector (usually 2 dimensional), indifferently to the dimension of the map's weights. 
