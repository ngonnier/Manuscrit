...
\section{Cartes auto-organisatrices}
...
\item Le prototype de la best matching unit est déplacé vers l'entrée $\inpx$, ainsi que les prototypes des noeuds voisins de $\bmu$ dans un rayon de voisinage défini à l'avance.
\end{enumerate}

L'algorithme de Kohonen repose donc sur à la fois un processus de compétition, avec la selection de la best matching unit de la carte, et un processus de coopération avec le déplacement des unités voisines de la Best Matching Unit. 
...
\item Elle conserve les densités
\end{itemize}
...
\draft{La carte de Kohonen se distingue d'autres algorithmes de quantification vectoriel par la topologie introduite par la carte dans l'ensemble des prototypes. Cette topologie dépend du voisinage utilisé par l'algorithme et de la dimension du support de la carte.
La plupart des algorithmes de SOM de la littérature utilisent comme support une grille en deux dimensions. L'indexation des noeuds est alors un ensemble de positions 2D. Le voisinage choisi est alors une distance en terme de nombre de noeuds de séparation (distance de Manhatthan) et est représenté sur différents supports en figure~\ref{fig:topo}. Une autre option est de considérer les distances euclidiennes entre positions des noeuds dans la carte.}
