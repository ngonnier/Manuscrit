...
%\begin{algorithm}
%  \caption{Learning iteration}\label{algo:iteration} 
%  \begin{algorithmic}
%  \State $\inpx\m{1}, ... , \inpx\m{K} \leftarrow (o^1, \cdots , o^K)\in \mathcal{D}^1 \times \cdot \times \mathcal{D}^K$ \Comment{An external input is presented to each map}
%  \State $t \leftarrow 0$
%  \State $ \forall i , \bmu\m{i} \leftarrow \argmax_p a\ext(\inpx\m{i},p)$ \Comment{In each map, $\bmu\m{i}$ is initialised from $a_e^i$}
%  \While {$\mathbf{\bmu}(t) \neq \mathbf{ \bmu}(t-1)$}\Comment{While no fixed point is reached}
%  \ForAll{Map i}
%  \State $\inpc\m{i}_1,...\inpc\m{i}_k \leftarrow$ BMUs from connected maps
%  \State Computation of $a_g^i$ (equation~\ref{activity})
%  \State $\hat{p}\m{i} \leftarrow \argmax_p a_g^i(p)$
%  \State $\bmu\m{i} \leftarrow \bmu\m{i} + min(\Delta, \lvert \hat{p}\m{i} - \bmu\m{i} \rvert) \times \sign(\hat{p}\m{i} - \bmu\m{i})$ \Comment{Move $\bmu$ of a fixed size step towards $\hat{p}$}
%  \EndFor
%  \State $t \leftarrow t+1$
%  \EndWhile
%  \State $\forall i, \w\ext\m{i}(p) \leftarrow \w\ext\m{i}(p) + H\ext(\bmu\m{i}, p)(\w\ext\m{i}(p) - \inpx\m{i})$ \Comment{Update external}
%  \State $ \forall i, \forall k, \w_{ck}\m{i}(p) \leftarrow \w_{ck}\m{i}(p) + H\cont(\bmu\m{i},p)(\w_{ck}\m{i}(p) - \inpc\m{i})$ \Comment{And contextual weights}
%  \end{algorithmic}
% \end{algorithm}

\section{Choix des paramètres}

Choix de hc, he, comparaison
